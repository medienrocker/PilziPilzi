...
#pragma warning restore CS4014
                    ...
                    if (_showSettings)
                    {
                        ...
                        AssetInventory.Config.doubleClickBehavior = EditorGUILayout.Popup(AssetInventory.Config.doubleClickBehavior, _doubleClickOptions);
                        GUILayout.EndHorizontal();
                        if (EditorGUI.EndChangeCheck()) AssetInventory.SaveConfig();
        ...
        private void PerformSearch(bool keepPage = false, bool ignoreExcludedExtensions = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(_searchPhrase))
            {
                ...
                if (phrase.StartsWith("=")) // expert mode
                {
                    if (phrase.Length > 1)
                    {
                        phrase = IOUtils.EscapeSQL(phrase);
