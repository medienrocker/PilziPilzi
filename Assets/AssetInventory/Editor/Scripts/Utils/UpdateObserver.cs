...
namespace AssetInventory
{
    public sealed class UpdateObserver
    {
        ...
        private async void ScanContinuously()
        {
            while (true)
            {
                if (_watcher != null && _watcher.EnableRaisingEvents && AssetInventory.Config.autoStopObservation)
                {
                    if (DateTime.Now - _lastObserverActivity > TimeSpan.FromSeconds(AssetInventory.Config.observationTimeout))
                    {
                        Stop();
                    }
                }
